...
import type { Trainers } from '../helpers/schema';
...
export type OutputType = Selectable<Trainers>[];

export const getTrainers = async (params: InputType, init?: RequestInit): Promise<OutputType> => {
  ...
  const response = await fetch(`/_api/trainers?${queryParams.toString()}`, {
    method: "GET",
    ...init,
    headers: {
  ...
  if (!response.ok) {
    const errorObject = superjson.parse(await response.text());
        throw new Error((errorObject as any)?.error || "Failed to fetch trainers");
  }
